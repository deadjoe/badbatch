...
fn create_disruptor_routes() -> Router {
    Router::new()
        ...
        .route("/:id/status", get(handlers::disruptor::get_disruptor_status))
...
mod tests {
    ...
    fn test_route_groups_not_empty() {
        assert!(!route_groups::SYSTEM.is_empty());
        assert!(!route_groups::DISRUPTOR_MANAGEMENT.is_empty());
        assert!(!route_groups::EVENT_PUBLISHING.is_empty());
        assert!(!route_groups::MONITORING.is_empty());
    }
    ...
    fn test_route_documentation() {
        ...
        assert!(docs.iter().any(|doc| doc.path == "/api/v1/disruptor" && doc.method == "POST"));
