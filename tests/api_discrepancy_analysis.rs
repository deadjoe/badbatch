...
mod api_discrepancy_tests {
    ...
    fn test_import_patterns_from_readme() {
        ...
        use badbatch::disruptor::{
            BlockingWaitStrategy, DefaultEventFactory, Disruptor, ProducerType,
        };

        // README.md lines 166-169 (Modern disruptor-rs API imports)
        use badbatch::disruptor::{
            build_single_producer, event_factory::ClosureEventFactory,
            simple_wait_strategy::BusySpin, RingBuffer,
        };
    ...
    fn test_discrepancy_wait_strategy_in_builder() {
        ...
        let _producer2 =
            build_single_producer(1024, MyEvent::default, simple_wait_strategy::busy_spin())
                .handle_events_with(|event, _seq, _end| {
                    event.value = 42;
                })
                .build();
