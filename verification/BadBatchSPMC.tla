--------------------------- MODULE BadBatchSPMC ---------------------------
(***************************************************************************)
(* Models BadBatch Disruptor Single Producer Multi Consumer (SPMC).       *)
(*                                                                         *)
(* This model verifies the correctness of BadBatch's single producer      *)
(* implementation with multiple consumers, based on the proven            *)
(* disruptor-rs model but adapted for BadBatch specifics.                 *)
(*                                                                         *)
(* The model verifies that:                                                *)
(* 1. No data races occur between producer and consumers                   *)
(* 2. All consumers eventually read all published events                   *)
(* 3. Events are processed in the correct order                           *)
(* 4. The system maintains memory safety and consistency                   *)
(***************************************************************************)

EXTENDS Naturals, Integers, FiniteSets, Sequences

CONSTANTS
  Writers,      (* Writer/publisher thread ids (should be single producer) *)
